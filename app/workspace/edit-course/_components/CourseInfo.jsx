...
const InfoCard = ({ icon, label, value }) => {
  return (
    <div className="bg-slate-800/50 border border-slate-700/80 rounded-xl p-4 flex items-center space-x-4 transition-all duration-300 hover:border-teal-400/80 hover:bg-slate-800/80">
...
const CourseInfo = ({ course, viewCourse }) => {
  ...
  return (
    <div className="w-full bg-slate-900/50 backdrop-blur-lg border border-slate-700/80 rounded-2xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
      
      {/* Generating Animation Overlay */}
      {isGenerating && (
        <div className="absolute inset-0 bg-slate-900 bg-opacity-95 flex flex-col items-center justify-center z-20 backdrop-blur-sm rounded-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="w-full aspect-square rounded-xl overflow-hidden border-2 border-slate-700/80">
            <Image
              src={'/useme2.jpg'}
              width={400}
              height={400}
              ...
              onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/400x400/1e293b/94a3b8?text=Course'; }}
            />
          </div>
        </div>
          ...
          <div>
            ...
            {!viewCourse ? (
              <button 
                ...
                className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-cyan-400/40 focus:outline-none focus:ring-4 focus:ring-cyan-300/50 transition-all duration-300 ease-in-out transform hover:-translate-y-1 flex items-center justify-center group disabled:bg-slate-600 disabled:shadow-none disabled:cursor-not-allowed disabled:transform-none"
              >
              ...
              <Link href={'/course/'+course?.cid} className="w-full">
                  <Button className={'w-full py-4 h-auto text-base bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-teal-400/40 transform hover:-translate-y-1 transition-all duration-300 cursor-pointer'}>
